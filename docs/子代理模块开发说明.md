...
当前系统没有独立 `AgentRegistry`；子代理以主 Agent 的 Function Tool 名称作为注册入口，按需创建。
...
- 原子工具：[mongo_query_tool.py](../AgentBI/src/tools/mongo_query_tool.py)、[send_email_tool.py](../AgentBI/src/tools/send_email_tool.py)
...
- 只做一个可复用动作，例如查询 Mongo、发邮件、写入 Excel、生成图片。
...
> `OfficeAgent` 负责根据用户要求创建或编辑 Office 文件，调用文件与 Office 原子工具，返回产物摘要；不负责直接访问 Mongo 或发送邮件。
...
在 `ChatAgent.tool_definitions()` 增加一个 `delegate_<domain>`，只传递高层任务说明，不把子代理内部 Tool 暴露给主模型。

```python
{
    "type": "function",
    "function": {
        "name": "delegate_office",
        "description": "将 Office 文件任务委派给 Office 子代理。",
        "parameters": {
            "type": "object",
...
随后在 `ChatAgent._stream_tool_loop()` 分发：

```python
if call["name"] == "delegate_office":
    yield {"type": "tool_started", "tool": "Office 子代理"}
    async for event in OfficeAgent().stream(client, model, temperature, instruction):
        yield event
    yield {"type": "tool_finished", "tool": "Office 子代理", "content": "Office 任务已处理"}
    return
```

当前阶段可以沿用该显式分支。子代理数量达到 3 个以上时，再抽取 `AgentRegistry`，用 `delegate_*` 名称映射 Agent 类和显示名称，避免不断扩张 `if` 分支。
...
不要传 API Key、SMTP 密码或无关的完整聊天记录。`ChatAgent._email_instruction()` 是当前的参考实现。
...
- 子代理若产生文件，应先返回简短路径/摘要；需要前端产物卡片时，再扩展新事件类型和 `ChatMessage` TypeScript 类型。
- 历史消息必须能只依靠 Mongo 中的 `timeline` 回放，不依赖浏览器内存。
...
- 当前主 Agent 的子代理分发仍是显式分支，不是通用注册表。
- 子代理执行仍在 Web 请求内；Office 渲染、视频生成等长任务应改为任务队列。
- 当前无鉴权和权限隔离；不要把此阶段的 Tool 边界误认为安全边界。
- 需要加入第三个子代理时，优先实现 `AgentRegistry`，并把注册表作为 `app.state` 生命周期对象管理。
