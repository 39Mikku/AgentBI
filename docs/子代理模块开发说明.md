...
`BilibiliAgent` 是“外部 CLI 查询 + 通用卡片 + 前端内嵌交互”的参考模块：

```text
delegate_bilibili
  -> BilibiliAgent
  -> search_videos / get_video_detail
  -> BilibiliClient
  -> card(kind=bilibili.video-list | bilibili.video)
  -> TimelineCard
  -> BilibiliPlayerModal
```

新增依赖不再为 `create_subagent()` 编写逐层 `if/else`。`ChatAgent` 传入以依赖 ID 为键的字典，注册项通过 `dependency="bilibili_client"` 声明所需对象。纯查询型 CLI 可以像本模块一样按请求启动隐藏子进程；需要健康检查、端口或会话状态的长驻服务仍应由 FastAPI lifespan 持有。

卡片的可交互地址应由稳定 ID 延迟构造。Bilibili 卡片持久化 BV 号，浏览器点击后才生成官方 iframe URL；这与音乐卡片只持久化歌曲 ID、播放时再解析 URL 的原则一致。
