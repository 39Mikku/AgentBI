...
import { fileToDataUrl } from '@/utils/image-source'
...
const busy = ref<'generating' | 'cutting' | 'naming' | ''>('')
const error = ref('')
const notice = ref('')
...
const canGenerate = computed(() => Boolean(subject.value.trim()) && !busy.value)
...
async function generate() {
  ...
  try {
    sheet.value = await generateImage({
      ...
      prompt: buildStickerPrompt({ subject: subject.value, style: effectiveStyle.value, includeText: includeText.value }),
      aspect_ratio: 'square',
...
async function recut() {
  if (!sheet.value || busy.value === 'cutting') return
  error.value = ''
  busy.value = 'cutting'
  try {
    stickers.value = await cutStickerSheet(sheet.value.url, cutOptions.value)
  ...
  <main class="emoji-workbench">
    ...
    <section class="press-grid">
      <aside class="control-rail">
        ...
        <label class="subject-field"><span>角色与表情主题</span><textarea v-model="subject" maxlength="2000" rows="4" placeholder="例如：粉色长发少女在聊天中的常用反应，活泼、傲娇、偶尔毒舌"></textarea><small>{{ subject.length }} / 2000</small></label>
      ...
      <section class="sheet-stage">
        <header><div><span>02</span><b>母版预览</b></div><small>{{ sheet ? `${sheet.model} / ${sheet.mode.toUpperCase()}` : 'MASTER SHEET / 1:1' }}</small></header>
        <div class="sheet-frame" :class="{ empty: !sheet }">
          <template v-if="sheet"><img :src="sheet.url" alt="生成的完整贴纸表" /><div class="crop-grid" aria-hidden="true"><i v-for="n in 16" :key="n"></i></div></template>
          <div v-else class="sheet-placeholder"><span>16</span><b>一张母版<br>十六个独立反应</b><small>填写左侧规格后开始生成</small></div>
          <div v-if="busy" class="stage-loader"><i></i><span>{{ statusLabel }}</span></div>
        </div>
        <footer><span>{{ sheet ? '原始母版已自动归入附件库' : 'PURE BACKGROUND / WIDE GUTTERS / AUTO CUT' }}</span><button :disabled="!sheet || Boolean(busy)" @click="recut">重新切割</button></footer>
      </section>
