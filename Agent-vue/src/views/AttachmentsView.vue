...
const filter = ref<'all' | 'generated' | 'uploaded' | 'docx'>('all')
const search = ref('')
...
const counts = computed(() => ({
  ...
  generated: assets.value.filter((item) => item.source === 'generated').length,
...
async function load() {
  ...
  try {
    ...
    if (filter.value === 'generated') filters.source = 'generated'
...
<template>
  <main class="asset-library">
    <header class="library-head">
      ...
      <h1>附件，<em>归档成册。</em></h1>
      <span>对话生成图片、上传图片与 DOCX 的统一索引。</span>
    </header>
    ...
    <section v-else class="library-empty"><b>00</b><h2>这里还没有附件</h2><p>在 Studio 上传文件或生成图片后，它们会汇总到这里。</p></section>
  </main>
</template>
