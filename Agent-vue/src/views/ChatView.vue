...
<template>
  <div class="workbench" :class="{ 'sidebar-open': menuOpen }">
    ...
    <div v-if="chat.isSyncing" class="sync-indicator" role="status" aria-live="polite"><i></i><span>{{ chat.syncMessage }}</span></div>
    <main class="dialogue">
...
.sync-indicator{position:fixed;z-index:50;inset:0;display:flex;align-items:center;justify-content:center;gap:10px;background:rgba(236,233,226,.52);backdrop-filter:blur(2px);font:600 11px 'DM Mono';letter-spacing:.08em;color:#242321}.sync-indicator span{padding:10px 14px;background:#171717;color:#f7f5f0;box-shadow:4px 4px 0 var(--acid)}.sync-indicator i{position:absolute;width:16px;height:16px;border:2px solid #171717;border-right-color:transparent;border-radius:50%;animation:sync-spin .72s linear infinite;transform:translateX(-78px)}@keyframes sync-spin{to{transform:translateX(-78px) rotate(360deg)}}
</style>
