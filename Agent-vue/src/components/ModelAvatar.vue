...
const props = withDefaults(defineProps<{ model?: string | null; size?: 'small' | 'regular' }>(), { size: 'regular' })
const brand = computed(() => resolveModelBrand(props.model))
</script>

<template>
  <span class="model-avatar" :class="size" :title="brand.name" :style="{ '--brand': brand.accent }">
